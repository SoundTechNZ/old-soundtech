...
class IWD_Opc_PaypalController extends Mage_Core_Controller_Front_Action
{
    ...
    public function returnAction()
    {
        ...
        $paypalCustomerData = Mage::getModel('opc/paypal_customer')->getPaypalCustomerDataByField(
            'payer_id',
            $profile->getUserId()
        );
        ...
        if (!$paypalCustomerData) {
            ...
            Mage::getSingleton('customer/session')->addSuccess(
                $this->__(
                    "Your %s account has been linked with your PayPal login.
                    <br />
                    <br />
                    You account details are displayed below. No financial information will ever be shared by PayPal.
                    <br />
                    <br />
                    Simply use Login with PayPal to login when you visit.", Mage::app()->getStore()->getFrontendName()
                )
            );
        } else {
    ...
    public function unlinkAction()
    {
        if ($this->_getSession()->getCustomer()->getId()) {
            ...
            $this->_getSession()->addSuccess(
                $this->_getHelper()->__(
                    "Your %s account is no longer linked with your PayPal login.",
    ...
    public function askLinkPostAction()
    {
        ...
        if (array_key_exists('send', $params)) {
            ...
            $isPaypalCustomerExists = Mage::getModel('opc/paypal_customer')->isPaypalCustomerExists(
                'customer_id',
                $customerId
            );
            ...
            if ($isPaypalCustomerExists) {
                $this->_getSession()->addError(
                    $this->_getHelper()->__(
                        "Your %s account is already linked with your PayPal login.",
                        Mage::app()->getStore()->getFrontendName()
                    )
                );
            ...
            Mage::getSingleton('customer/session')->addSuccess(
                $this->__(
                    ...
                    You account details are displayed below. No financial information will ever be shared by PayPal.
                    <br /><br />
                    Simply use Login with PayPal to login when you visit.",
                    Mage::app()->getStore()->getFrontendName()
                )
            );
        } else {
            ...
            if ($customerId) {
                ...
                $this->_getSession()->addError(
                    $this->_getHelper()->__(
                        ...
                        Please enter the %s credentials to link the account.",
                        Mage::app()->getStore()->getFrontendName()
                    )
                );
                $this->_redirect($askLinkUrl);
            ...
            try {
                ...
                Mage::getSingleton('customer/session')->addSuccess(
                    $this->__(
                        ...
                        <br /><br />
                        You account details are displayed below.
                        No financial information will ever be shared by PayPal.
                        <br /><br />Simply use Login with PayPal to login when you visit.",
                        Mage::app()->getStore()->getFrontendName()
                    )
                );
            } catch (Exception $e) {
    ...
    {
        return '<script type="text/javascript">window.opener.location.href="' . $url . '";self.close();</script>';
    }
}