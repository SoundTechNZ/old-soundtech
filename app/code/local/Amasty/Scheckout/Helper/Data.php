...
//                exit;
    ...
    function skipAmGiftCardSection()
    {
    ...
    public function getExtraField($_field)
    {
        $ret = "";
...
//            
    ...
    public function displayExtrafields($fields)
    {
        ...
        if (Mage::helper('core')->isModuleEnabled("Amasty_Orderattr")) {
            $oaFields = $this->_collectExtraFields($fields);
            $attributes = $this->_getOrderAttributeCollection($oaFields);
            foreach ($attributes as $attribute) {
                $attributeCode = $attribute->getAttributeCode();
                foreach ($fields as $field) {
                    if ($field['field_key'] == $attributeCode) {
                        $html .= $this->getExtraField($field);
                    }
                }
            }
        }
    ...
    protected function _collectExtraFields($fields = array(), $typeField = 'oa')
    {
        $extraFields = array();

        foreach ($fields as $key => $field) {
            if (strstr($field['field_db_key'], $typeField)) {
    ...
    protected function _getOrderAttributeCollection($extraFields = array())
    {
